...
class CommunityClusteringAlgo(ABC):
    ...
    def save_community_labels(self, suffix=''):
        # save community labels from anndata file
        self.adata.obs[f'tissue_{self.method_key}'].to_csv(os.path.join(self.dir_path, f'{self.filename}{suffix}.csv'))
        logging.info(f'Saved community labels after clustering as a part of original anndata file to {self.filename}{suffix}.csv')
